...
//    height: 768
    ...
    Label {
        ...
        text: qsTr("Shipping Create/Edit")
    ...
    Rectangle {
        ...
        color: "#ffffff"
        border.width: order_input.activeFocus ? height*5/100 : height*2.5/100
        border.color: order_input.activeFocus ? "#fd8a15" : "#f1f1f1"
        ...
        TextInput {
            ...
            text: rowNum > -1 ? shippingTable_model.getData(rowNum,2): ""
    ...
    Label {
        ...
        color: "#14428b"
        text: qsTr("Ship#")
        anchors.horizontalCenterOffset: -parent.width*33/100
        verticalAlignment: Text.AlignVCenter
        font.bold: true
        font.pixelSize: height*55/100
        horizontalAlignment: Text.AlignHCenter
    ...
    Rectangle {
        ...
        TextInput {
            ...
            font.pixelSize: parent.height*50/100
            echoMode: TextInput.Normal
            text: rowNum > -1 ? shippingTable_model.getData(rowNum,3): ""
            validator: RegExpValidator{regExp: /^[a-zA-Z0-9 ]+$/}
            horizontalAlignment: Text.AlignLeft
        }
        anchors.horizontalCenterOffset: -parent.width*4/100
        border.color: ship_input.activeFocus ? "#fd8a15" : "#f1f1f1"
        border.width: ship_input.activeFocus ? height*5/100 : height*2.5/100
        anchors.horizontalCenter: parent.horizontalCenter
    }
    ...
    RoundButton {
        ...
        onClicked: {
            ...
            order_input.clear()
            ship_input.clear()
            if(order_input.text.length > 0 && ship_input.text.length > 0)
            {
                 if(!edit_fl){

                     if(shippingTable_model.insertData(order_input.text,ship_input.text,userId))
                     {
                         taskcreate_page.signalExit()
                     }
                 }
                 ...
                 {
                     var pri_key = shippingTable_model.getData(rowNum,0)
                     if(shippingTable_model.updateData(order_input.text,ship_input.text,userId,pri_key))
                     {
                         taskcreate_page.signalExit()
                     }
                 }
